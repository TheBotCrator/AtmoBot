...
global.Status = `${Prefix}help | Sector Welcome Assistant. BTM2.5FG`; // Status of the Bot.
...
Bot.on("message", Message => {
	...
	if (Records[Message.guild.id]) { 
		...
		if (Suggestions.USEABLE) {
			let RecordChannel = Message.guild.channels.find(`name`, Suggestions.Record)
				...
				if (SuggestionChannel) {
					if (SuggestionChannel.id === SentMessageChannel.id) {
						...
						let SecondEmbed = new Discord.RichEmbed()
							.setColor("6e00ff")
							.setTimestamp()
							.addField("Suggested By", `${Message.author.username} | ${Message.author.id}`)
							.addField("Suggested", `${Message}`);
						let PersonMentioned = Message.channel.send(`${Message.user.username}`)
